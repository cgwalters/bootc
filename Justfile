...
# ============================================================================
# Development VM workflow (bcvk-based)
# ============================================================================

# Name for the development VM
_dev_vm_name := "bootc-dev"
_dev_vm_label := "bootc.dev=1"
_sysext_name := "bootc"
_sysext_dir := "target/sysext"

# Build a systemd-sysext from the RPM packages
[group('dev')]
sysext: package
    #!/bin/bash
    set -xeuo pipefail
    sysext_path="{{_sysext_dir}}/{{_sysext_name}}"
    
    # Clean and create the sysext directory structure
    rm -rf "${sysext_path}"
    mkdir -p "${sysext_path}"
    
    # Extract the bootc RPM into the sysext directory
    # We only want the /usr files for a sysext
    shopt -s nullglob
    rpms=(target/packages/bootc-*.rpm)
    shopt -u nullglob
    if [[ ${#rpms[@]} -eq 0 ]]; then
        echo "No bootc RPMs found in target/packages" >&2
        exit 1
    fi
    for rpm in "${rpms[@]}"; do
        if [[ "${rpm}" == *".src.rpm" ]]; then
        ...
        echo "Extracting ${rpm}..."
        rpm2cpio "${rpm}" | (cd "${sysext_path}" && cpio -idmv)
    done
    
    # Remove non-/usr content (sysexts only overlay /usr and /opt)
    # Keep /usr, remove everything else at the top level
    shopt -s nullglob
    for dir in "${sysext_path}"/*; do
        dir_name=$(basename "${dir}")
        if [[ "${dir_name}" != "usr" && "${dir_name}" != "opt" ]]; then
            echo "Removing ${dir} (sysexts only support /usr and /opt)"
            rm -rf "${dir}"
        fi
    done
    shopt -u nullglob
    ...
    # The extension-release.<name> file must match the sysext directory name
    # and contain at minimum ID= matching the host OS
    mkdir -p "${sysext_path}/usr/lib/extension-release.d"
    release_file="${sysext_path}/usr/lib/extension-release.d/extension-release.{{_sysext_name}}"
    # Use _any to relax OS ID matching - this sysext is for development/testing
    printf '%s\n' "ID=_any" "EXTENSION_RELOAD_MANAGER=1" > "${release_file}"
    
    echo "Sysext created at ${sysext_path}"
    echo "Contents:"
    find "${sysext_path}" -type f | head -20
    echo "..."

# Launch or sync to a development VM with the sysext mounted
[group('dev')]
bcvk-up: sysext
    #!/bin/bash
    set -xeuo pipefail
    ...
    if bcvk libvirt list {{_dev_vm_name}} --format=json 2>/dev/null | jq -e '.[0]' >/dev/null 2>&1; then
        echo "Development VM '{{_dev_vm_name}}' exists, syncing sysext..."
        just bcvk-sync
    else
        echo "Creating development VM '{{_dev_vm_name}}'..."
        just _bcvk-create
    fi

# Create the development VM (internal helper)
[group('dev')]
_bcvk-create: build
    #!/bin/bash
    set -xeuo pipefail
    
    sysext_path=$(realpath "{{_sysext_dir}}")
    
    # Create VM with virtiofs mount for the sysext directory
    # The bind mount will be at /run/extensions/bootc which is where systemd-sysext looks
    # Note: secure-boot-keys are generated by _keygen which is a dependency of `build`
    bcvk libvirt run \
        --name={{_dev_vm_name}} \
        --replace \
        --label={{_dev_vm_label}} \
        --bind="${sysext_path}/{{_sysext_name}}:/run/extensions/{{_sysext_name}}" \
        --secure-boot-keys=target/test-secureboot \
        --ssh-wait \
        {{base_img}}
    
    # Enable and start the sysext
    echo "Enabling sysext in VM..."
    bcvk libvirt ssh {{_dev_vm_name}} -- systemd-sysext refresh
    bcvk libvirt ssh {{_dev_vm_name}} -- systemd-sysext status
    
    echo ""
    echo "Development VM is ready!"
    echo "  SSH:  just bcvk-ssh"
    echo "  Sync: just bcvk-sync"
    echo "  Stop: just bcvk-down"

# Sync the sysext to the running VM and reload
[group('dev')]
bcvk-sync: sysext
    #!/bin/bash
    set -xeuo pipefail
    
    # Verify the VM is running before attempting to sync
    if ! bcvk libvirt list {{_dev_vm_name}} --format=json 2>/dev/null | jq -e '.[0].state == "running"' >/dev/null 2>&1; then
        echo "Error: Development VM '{{_dev_vm_name}}' is not running. Use 'just bcvk-up' to start it." >&2
        exit 1
    fi
    
    # The sysext is already mounted via virtiofs, so changes are visible immediately
    # We just need to tell systemd-sysext to reload
    echo "Refreshing sysext in VM..."
    bcvk libvirt ssh {{_dev_vm_name}} -- systemd-sysext refresh
    bcvk libvirt ssh {{_dev_vm_name}} -- systemd-sysext status
    
    # Show the new bootc version
    echo ""
    echo "Current bootc version in VM:"
    bcvk libvirt ssh {{_dev_vm_name}} -- bootc --version

# SSH into the development VM
[group('dev')]
bcvk-ssh *ARGS:
    bcvk libvirt ssh {{_dev_vm_name}} -- {{ARGS}}

# Stop and remove the development VM
[group('dev')]
bcvk-down:
    bcvk libvirt rm --stop --force {{_dev_vm_name}} || echo "Note: VM '{{_dev_vm_name}}' was not running or did not exist"

# Show status of the development VM
[group('dev')]
bcvk-status:
    @bcvk libvirt list {{_dev_vm_name}} || echo "No development VM '{{_dev_vm_name}}' found"
