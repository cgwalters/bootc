...
fn parse_composefs_candidates(cmdline: &str) -> Result<Vec<ComposefsCmdline<Sha512HashValue>>> {
    ...
    for parameter in Cmdline::from(cmdline).iter() {
        ...
        let (seen, key_name) = if &*key == KARG_COMPOSEFS_DIGEST {
            (&mut seen_digest, KARG_COMPOSEFS_DIGEST)
        } else if &*key == KARG_V2 {
            (&mut seen_v2, KARG_V2)
        } else {
            continue;
        ...
        if *seen {
            anyhow::bail!("duplicate {key_name}= composefs kernel argument");
        }
        *seen = true;

        // ComposefsCmdline owns the format and digest validation.  Rebuild a
        // single canonical parameter from the kernel parser's dequoted value.
        let parameter = format!("{key_name}={value}");
...
mod tests {
    ...
    fn test_composefs_candidate_parser_cases() {
        ...
        let duplicate_cases = [
            format!("{} {}", v1(first.clone(), false), v1(second.clone(), false)),
            format!("{} {}", v2(first.clone(), false), v2(second.clone(), false)),
            format!("{} {}", v1(first.clone(), false), v1(first.clone(), false)),
