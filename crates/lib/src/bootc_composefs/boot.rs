    ...
    let (root_path, esp_device, mut cmdline_refs, bootloader) = match setup_type {
        BootSetupType::Setup((root_setup, state, postfetch)) => {
            ...
            let composefs_cmdline =
                ComposefsCmdline::build(&id_hex, state.composefs_options.allow_missing_verity, repo.erofs_version());
        ...
        BootSetupType::Upgrade((storage, booted_cfs, host)) => {
            ...
            let cfs_cmdline =
                ComposefsCmdline::build(&id_hex, booted_cfs.cmdline.allow_missing_fsverity, repo.erofs_version())
                    .to_string();
